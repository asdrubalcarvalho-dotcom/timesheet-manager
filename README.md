...
**TimePerk Cortex - The brain behind your team's time** is a lightweight **Timesheet and Expense Management System** built with **Laravel 11 API + React 18 SPA**, fully containerized with **Docker Compose**.  
...
- Submitting and approving **project-related expenses** via data grid
...
- **Rate limiting** and security middleware protection on all API endpoints
- Viewing **interactive dashboards** with pending approvals and summaries  
...
- **Laravel Policies**: TimesheetPolicy and ExpensePolicy for ownership and status-based rules
...
- **Ownership Rules**: Technicians access only their records, Managers/Admins access all
- **Status Protection**: Cannot edit/delete approved records (Admin override available)

### Role-Specific Validation Rules (Updated 2025-11-06)
...
| **Manager** | ✅ Own projects + own records | ✅ Can approve/reject timesheets from managed projects | ✅ Assigned via `projects.manager_id` |
| **Admin** | ✅ All system records | ✅ Can validate all timesheets | ✅ Full system access |

**Implementation Details**:
- **Database Relationship**: `projects.manager_id` → `users.id` (Foreign Key)
- **Policy Validation**: `TimesheetPolicy::approve()` checks project ownership
...
|---------|-----------|------|-------------|
| **Backend API** | Laravel 11 + PHP-FPM 8.3 | 80 | REST API with Sanctum + Spatie Permission |
| **Frontend SPA** | React 18 + Vite + Node 20 | 3000 | Calendar & grid interface with authorization |
| **Database** | MySQL 8.0 | 3306 | Persistent data storage with constraints |
| **Cache** | Redis Alpine | 6379 | Session & cache storage |
